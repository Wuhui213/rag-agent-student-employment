<template>
  <div class="chat-container">
    ...
    <aside class="sidebar" :class="{ 'collapsed': sidebarCollapsed }">
      ...
      <button class="new-chat-btn" @click="newChat" v-if="!sidebarCollapsed">
      ...
      <div class="session-list" v-if="!sidebarCollapsed">
        ...
        <div 
          v-for="session in sessions" :key="session.session_id"
          class="session-item" :class="{ active: currentSessionId === session.session_id }"
          @click="loadSession(session.session_id)"
        >
          <span class="session-icon">📝</span>
          <span class="session-text">{{ formatSessionTime(session.last_active) }}</span>
        </div>
    ...
    <div class="main-area">
      <header class="header">
        ...
        <span class="user-greeting">{{ greetingText }}</span>
      </header>
      ...
      <div class="messages" ref="messagesRef">
        ...
        <div v-if="messages.length === 0" class="empty-state">
          ...
          <div class="suggestions">
            <span class="suggestion" @click="quickAsk('📊 画一个就业率柱状图')">📊 就业率分析</span>
            <span class="suggestion" @click="quickAsk('📈 分析实习经验对就业的影响')">📈 实习vs就业</span>
            <span class="suggestion" @click="quickAsk('🔍 查询GPA最高的前10名学生')">🔍 数据查询</span>
            <span class="suggestion" v-if="userDept" @click="quickAsk(`🎓 分析${userDept}学生的就业特征`)">🎓 我的学院</span>
          </div>
        </div>
      ...
      <div class="input-area">
        <button class="upload-btn" @click="triggerUpload" :disabled="isTyping || isUploading" title="上传数据文件">
          <span v-if="isUploading">⏳</span>
          <span v-else>📎</span>
        </button>
        <input type="file" ref="fileInputRef" @change="handleFileUpload" accept=".csv,.xlsx,.xls" style="display:none" />
        ...
        <div class="modal-body">
          <div class="setting-item">
            ...
            <div class="setting-options">
              <button v-for="opt in detailOptions" :key="opt.value"
                class="setting-option" :class="{ active: settings.detail_level === opt.value }"
          ...
          <div class="setting-item">
            <label>📊 默认图表类型</label>
            <div class="setting-options">
              <button v-for="opt in chartOptions" :key="opt.value"
                class="setting-option" :class="{ active: settings.default_chart_type === opt.value }"
...
const settings = ref({ detail_level: 'normal', default_chart_type: 'bar', theme: 'light' })
...
function renderChart(msgId, chartOption) {
  nextTick(() => {
    ...
    // 监听窗口resize
    const onResize = () => { if (chartInstances[msgId]) chartInstances[msgId].resize() }
    window.addEventListener('resize', onResize)
    // 用ResizeObserver监听容器大小变化
    if (typeof ResizeObserver !== 'undefined') {
      const ro = new ResizeObserver(() => { if (chartInstances[msgId]) chartInstances[msgId].resize() })
      ro.observe(el)
    }
  })
}

// 重新渲染所有图表（在Vue可能重建DOM后调用）
...
function tryParseAnalysisContent(text) {
  ...
  try {
    const obj = JSON.parse(cleaned)
    // 只有包含至少一个分析字段（summary/analysis/conclusion）的才是分析内容
  ...
  try {
    ...
    if (data.code === 200 && data.data) {
      ...
      nextTick(() => {
        messages.value.forEach(msg => {
          if (msg.chartOption && msg._id) {
            renderChart(msg._id, msg.chartOption)
          }
        })
      })
    }
  ...
  try {
    ...
    if (result.code === 200) {
      ...
      if (importInfo?.success) {
        messages.value.push({ role: 'ai', content: `🎉 太棒了！文件 **${result.filename}** 已上传成功，导入了 ${importInfo.rows} 条数据～现在可以问我关于这些数据的问题啦！`, _id: makeMsgId() })
      } else {
        messages.value.push({ role: 'ai', content: `📎 文件 **${result.filename}** 已上传，但数据导入遇到了些问题：${importInfo?.error || '未知错误'} 😅`, _id: makeMsgId() })
      }
      scrollToBottom()
    } else {
...
<style scoped>
.chat-container { width: 100%; height: 100vh; display: flex; background: #f8f9ff; }
...
.sidebar-toggle {
  ...
  background: none; border: none; color: rgba(255,255,255,0.3); font-size: 16px; cursor: pointer;
}
...
.new-chat-btn {
  ...
  display: flex; align-items: center; gap: 8px; justify-content: center; transition: all 0.3s;
}
.new-chat-btn:hover { background: linear-gradient(135deg, rgba(102,126,234,0.35), rgba(196,77,255,0.35)); border-color: rgba(255,255,255,0.4); }

.session-list { flex: 1; overflow-y: auto; padding: 4px 0; }
.session-title { padding: 12px 16px 8px; font-size: 11px; color: rgba(255,255,255,0.3); text-transform: uppercase; letter-spacing: 1px; }
.session-item {
  display: flex; align-items: center; gap: 8px; padding: 10px 16px; cursor: pointer;
  transition: background 0.2s; font-size: 13px; color: rgba(255,255,255,0.6); border-radius: 8px; margin: 2px 8px;
}
.session-item:hover { background: rgba(255,255,255,0.08); }
.session-item.active { background: rgba(102,126,234,0.25); color: #fff; }
.session-empty { padding: 16px; font-size: 12px; color: rgba(255,255,255,0.25); text-align: center; }

.sidebar-footer { padding: 12px; border-top: 1px solid rgba(255,255,255,0.08); display: flex; flex-direction: column; gap: 4px; }
.settings-btn, .logout-btn {
  padding: 10px 12px; background: none; border: none; color: rgba(255,255,255,0.5);
  font-size: 13px; cursor: pointer; border-radius: 8px; text-align: left; transition: all 0.2s;
}
.settings-btn:hover, .logout-btn:hover { background: rgba(255,255,255,0.08); color: #fff; }
...
.header {
  ...
  padding: 14px 24px; background: #fff; box-shadow: 0 2px 12px rgba(102,126,234,0.08); flex-shrink: 0;
}
.header-left { display: flex; align-items: center; gap: 12px; }
.bot-mascot { font-size: 32px; animation: float 3s ease-in-out infinite; }
@keyframes float { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-6px)} }
.title { font-size: 18px; font-weight: 700; color: #333; margin: 0; }
.bot-status { font-size: 12px; color: #91CC75; }
.user-greeting { font-size: 14px; color: #666; background: #f8f9ff; padding: 8px 16px; border-radius: 20px; }
...
.mascot-bounce { font-size: 80px; margin-bottom: 20px; animation: bounce 2s ease infinite; }
@keyframes bounce { 0%,20%,50%,80%,100%{transform:translateY(0)} 40%{transform:translateY(-20px)} 60%{transform:translateY(-10px)} }
.welcome-title { font-size: 26px; font-weight: 600; color: #333; margin-bottom: 8px; }
.welcome-sub { font-size: 15px; color: #888; margin-bottom: 28px; max-width: 400px; line-height: 1.6; }

.suggestions { display: flex; gap: 12px; flex-wrap: wrap; justify-content: center; }
.suggestion {
  padding: 12px 20px; font-size: 14px; background: #fff; border-radius: 24px;
  box-shadow: 0 2px 12px rgba(102,126,234,0.12); cursor: pointer; transition: all 0.3s;
  border: 1px solid rgba(102,126,234,0.1);
}
.suggestion:hover { transform: translateY(-3px); box-shadow: 0 6px 20px rgba(102,126,234,0.2); background: #f0f0ff; }

/* 消息 */
.message-wrapper { display: flex; gap: 12px; margin-bottom: 20px; align-items: flex-start; animation: fadeIn 0.3s ease; }
...
.message-avatar.ai-avatar { background: linear-gradient(135deg, #667eea, #764ba2); font-size: 20px; }

.message-content {
  max-width: 70%; padding: 14px 18px; border-radius: 16px; line-height: 1.7; font-size: 15px;
}
...
.typing-dot { width: 6px; height: 6px; background: #667eea; border-radius: 50%; animation: pulse 1.4s infinite; }
...
.input-area {
  ...
  background: #fff; box-shadow: 0 -2px 12px rgba(102,126,234,0.06); flex-shrink: 0; align-items: center;
}
.upload-btn {
  width: 42px; height: 42px; font-size: 20px; background: #f0f0ff; border: none;
  border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; flex-shrink: 0;
  transition: all 0.3s;
}
.upload-btn:hover:not(:disabled) { background: #667eea; color: #fff; transform: scale(1.05); }
.upload-btn:disabled { background: #eee; cursor: not-allowed; }

.input-area .input {
  flex: 1; padding: 14px 20px; font-size: 15px; border: 2px solid #e8e8ff;
  border-radius: 28px; outline: none; transition: all 0.3s; background: #fafaff;
}
.input-area .input:focus { border-color: #667eea; box-shadow: 0 0 0 4px rgba(102,126,234,0.1); }

.send-btn {
  padding: 12px 28px; font-size: 15px; font-weight: 600; color: #fff;
  background: linear-gradient(135deg, #667eea, #764ba2); border: none; border-radius: 28px;
  cursor: pointer; transition: all 0.3s; flex-shrink: 0;
}
.send-btn:hover:not(:disabled) { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(102,126,234,0.35); }
...
.modal-content { width: 460px; background: #fff; border-radius: 20px; overflow: hidden; box-shadow: 0 20px 60px rgba(0,0,0,0.2); }
.modal-header { display: flex; justify-content: space-between; align-items: center; padding: 24px; border-bottom: 1px solid #f0f0ff; }
.modal-header h3 { font-size: 18px; font-weight: 600; margin: 0; }
.modal-close { background: none; border: none; font-size: 20px; color: #999; cursor: pointer; width: 32px; height: 32px; border-radius: 50%; display: flex; align-items: center; justify-content: center; }
.modal-close:hover { background: #f5f5f5; }
.modal-body { padding: 24px; }
.setting-item { margin-bottom: 24px; }
.setting-item label { display: block; font-size: 14px; font-weight: 600; color: #333; margin-bottom: 12px; }
.setting-options { display: flex; gap: 10px; flex-wrap: wrap; }
.setting-option {
  padding: 10px 20px; border: 2px solid #e8e8ff; border-radius: 12px;
  background: #fff; font-size: 14px; cursor: pointer; transition: all 0.3s;
}
.setting-option.active { border-color: #667eea; background: #f0f0ff; color: #667eea; font-weight: 600; }
.setting-option:hover:not(.active) { border-color: #c4c4ff; }
.modal-footer { padding: 16px 24px; border-top: 1px solid #f0f0ff; text-align: right; }
.save-btn {
...
.save-btn:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(102,126,234,0.3); }
</style>

<style>
/* Markdown全局样式 - 用于v-html渲染的表格 */
.markdown-body { color: #333; line-height: 1.7; font-size: 14px; }
.markdown-body h1 { font-size: 20px; font-weight: 700; color: #4a5568; margin: 16px 0 10px; border-bottom: 2px solid #e2e8f0; padding-bottom: 6px; }
.markdown-body h2 { font-size: 17px; font-weight: 700; color: #5a67d8; margin: 14px 0 8px; }
.markdown-body h3 { font-size: 15px; font-weight: 600; color: #667eea; margin: 12px 0 6px; }
.markdown-body h4 { font-size: 14px; font-weight: 600; color: #718096; margin: 10px 0 5px; }
.markdown-body p { margin-bottom: 10px; }
.markdown-body ul, .markdown-body ol { padding-left: 22px; margin-bottom: 10px; }
.markdown-body li { margin-bottom: 4px; }
.markdown-body code { background: #f0f0ff; padding: 2px 6px; border-radius: 4px; font-size: 13px; color: #667eea; }
.markdown-body pre { background: #f5f5ff; padding: 12px; border-radius: 10px; overflow-x: auto; }
.markdown-body strong { color: #667eea; font-weight: 600; }
.markdown-body blockquote { border-left: 4px solid #667eea; padding: 8px 14px; margin: 10px 0; background: #f7f7ff; border-radius: 0 8px 8px 0; color: #555; }
.markdown-body hr { border: none; border-top: 1px solid #e2e8f0; margin: 14px 0; }
/* 表格样式 */
.markdown-body table { width: 100%; border-collapse: collapse; margin: 12px 0; border-radius: 8px; overflow: hidden; box-shadow: 0 1px 4px rgba(0,0,0,0.08); font-size: 13px; }
.markdown-body thead { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); }
.markdown-body th { padding: 10px 14px; text-align: left; color: #fff; font-weight: 600; font-size: 13px; white-space: nowrap; }
.markdown-body td { padding: 9px 14px; border-bottom: 1px solid #edf2f7; color: #4a5568; }
.markdown-body tbody tr:nth-child(even) { background: #f7fafc; }
.markdown-body tbody tr:hover { background: #edf2f7; }
